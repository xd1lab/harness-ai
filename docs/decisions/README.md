...
| [0032](0032-estimator-doomloop-durable-approvals.md) | Three capability fixes: (1) the local token estimator (`tokencount.go`) now counts **all** token-bearing `ContentPart` variants — `Text`, `ToolResult.Content`, `ToolCall` name + JSON-marshaled args, `Thinking` — not just text (images excluded), so self-hosted/no-count runs with large tool outputs still trip threshold compaction; (2) doom-loop **terminates** via a new `domain.ErrorDoomLoop` reason mapped to the EXISTING `TERMINATION_SUBTYPE_ERROR_DURING_EXECUTION` wire subtype (NO proto change), default-on `DoomLoopThreshold=3` applied in `NewLoop` (0 ⇒ default 3, negative ⇒ explicit-disable; the `tools.go` guard moves from `<=0` to `<0`), tripping BEFORE dispatching the Nth identical batch via the existing `turnTerminal → finish` return contract; (3) **un-collapses** the approval ask into a distinct sealed `ApprovalRequested` event (vs the original `event.go` note that the family was modeled as the single `PermissionDecided`), appended BEFORE the gate blocks across every exhaustive codec/projection/read-plane/agentctx/dev-store site, with recovery classifying a suspended approval distinctly and resume achieving the **TARGET re-raise** level (bounded by `ResumeApprovalTimeout`, falling back to a durable-auditable close on an unattended restart) | Accepted |
| [0034](0034-signed-audit-checkpoints-and-siem-export.md) | Signed audit checkpoints + SIEM export (tamper-PROOF): an Ed25519-signed, GLOBAL checkpoint chain over the events' `content_hash` leaves anchored by a key held OUTSIDE the events DB (`checkpoint_hash = SHA-256(prev ‖ SHA-256(L1‖..‖Ln))`, versioned genesis), stored in the additive/forward-only/RLS-exempt/append-only `audit_checkpoints` (migration 0010, `ON CONFLICT (covers_to_global_id)` idempotency) by an operator-tier signer projector (own `audit-checkpoint` subscription, crash-safe unanchored-tail re-read, disabled-with-loud-WARN when no `BOLTROPE_AUDIT_SIGNING_KEY` — no ephemeral key); operator-tier `VerifyAuditCheckpoints` (recompute + prev-link + Ed25519 verify on the owner connection, NOT tenant-RLS) surfaced as `harnessctl audit verify-checkpoints` (no proto change); a descriptors+hashes-only SIEM export projector (own `siem-export` subscription, file/NDJSON + HTTP sinks, NEVER raw payload); all OFF the hot path, signer+SIEM are OPERATOR-TIER (plain net/http, NOT the egress broker — ADR-0013 governs only model-influenced channels); upgrades ADR-0033 from tamper-EVIDENT to tamper-PROOF | Accepted |
| [0033](0033-tamper-evident-hash-chain.md) | Tamper-evident audit log: a per-event `content_hash` (SHA-256 over the EXACT stored `events.payload` json.Marshal bytes) + a per-session SHA-256 hash-**CHAIN** `chain_hash = SHA256(prev‖content)` with a session-derived genesis (`"boltrope-audit-genesis-v1:"+session_id`), folded at **append-time** in the single-writer tx with `sessions.chain_head` as the running head; migration **0009** additive/nullable/forward-only (no `.down`, no backfill — pre-0009 rows stay UNCHAINED, RLS unaffected); a single pgx-free `domain/hashchain.go` shared by the prod store and `boltrope-dev` (parity by construction); side-effect-free RLS-scoped `VerifyChainIntegrity` recompute-and-compare returning `{Valid,FirstBadSeq,Reason,Checked}` (content-mismatch vs broken-link, leading NULL-hash prefix skipped); additive proto only — new `VerifySessionIntegrity` RPC + `EventDescriptor.content_hash=12`/`chain_hash=13`, wired on gRPC + REST (`GET /v1/sessions/{id}/integrity`) + MCP (`verify_session_integrity`, the 12th tool). Explicitly tamper-**EVIDENT** not tamper-**PROOF** (signed checkpoints + SIEM/WORM export are the Batch-5B follow-on that anchors the head outside the DB), and verify re-marshals the decoded payload — sound for the closed v1 event set at the current `schema_version`; a raw-stored-bytes verify path is the hardening follow-on | Accepted |
