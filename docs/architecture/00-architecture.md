...
> **Date:** 2026-06-10
> **Project:** Boltrope (`github.com/xd1lab/harness-ai`) — a provider-portable, event-sourced AI agent harness.
...
- **Dev fallback fails closed.** A static-cert mTLS provider exists behind the `infra/server` port for local `docker compose`. It **refuses to start unless `BOLTROPE_DEV_INSECURE=1`** is set, logs a loud warning, and generates ephemeral certs at startup (none committed). As built it is **env-gated, not compiled out**: the path is present in every binary but inert unless the env var is explicitly set; the **SPIRE Workload API wiring** is what is build-tag-controlled (release images build with `-tags spire`; ADR-0013 §Amendment). **Startup assertion:** if not in dev mode, the SPIFFE provider MUST be present or the process exits. This prevents a silent production downgrade to static certs.
...
- **Single module** (`module github.com/xd1lab/harness-ai`). Services that share the normalized `llm` types, the generated protobuf stubs, and the platform bootstrap are simplest to version and refactor atomically in one module. Multi-module adds `go.work`/replace friction with no benefit at this size.
...
```
boltrope/
├── go.mod                      # single module: github.com/xd1lab/harness-ai
├── go.sum
