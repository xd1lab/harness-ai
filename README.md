...
The event log isn't just append-only by convention — it's **cryptographically chained**, so a later mutation of any stored event is *detectable*. At append time, inside the same single-writer transaction that already enforces optimistic concurrency, lease fencing, and `request_id` idempotency, each event gets a **`content_hash`** (SHA-256 over the exact stored payload bytes) and a per-session **`chain_hash`** = `SHA256(prev_chain_hash ‖ content_hash)`, folded in `seq` order from a session-derived genesis. The exact bytes the hash is taken over are stored **verbatim** in a raw `payload_canonical` column (a `BYTEA`, not JSONB — so Postgres never normalizes whitespace, key order, or duplicate keys), and verify hashes those raw stored bytes *directly*. That makes structural tampering of the stored payload — reordering keys, adding whitespace, or injecting an extra key — change the hashed bytes and get caught, not just value changes. The running head lives on `sessions.chain_head`; the chain is **per-session** (it aligns with seq contiguity, RLS, and the session as the audit unit) ([ADR-0033](docs/decisions/0033-tamper-evident-hash-chain.md)).
