...
// BashTool runs a shell command inside the session workspace via
...
// NewBashTool returns a [BashTool] backed by ws.
func NewBashTool(ws app.Workspace) *BashTool { return &BashTool{ws: ws} }
...
// Execute runs args["command"] via 'sh -c' inside the workspace. The supplied
// ctx carries cancellation that the workspace maps to a process-group/cgroup kill
// of the in-sandbox tree (architecture §9.3); a killed or non-zero exit is
// returned as an error observation.
func (t *BashTool) Execute(ctx context.Context, _ string, args map[string]any) (domain.Observation, error) {
...
// GlobTool finds files matching a glob pattern inside the workspace. It is
// read-only with no network egress and runs via [app.Workspace.Exec].
type GlobTool struct {
	ws app.Workspace
}

// NewGlobTool returns a [GlobTool] backed by ws.
func NewGlobTool(ws app.Workspace) *GlobTool { return &GlobTool{ws: ws} }
...
func (t *GlobTool) Execute(ctx context.Context, _ string, args map[string]any) (domain.Observation, error) {
...
// GrepTool searches file contents in the workspace for a regular expression. It
// is read-only with no network egress and runs via [app.Workspace.Exec].
type GrepTool struct {
	ws app.Workspace
}

// NewGrepTool returns a [GrepTool] backed by ws.
func NewGrepTool(ws app.Workspace) *GrepTool { return &GrepTool{ws: ws} }
...
func (t *GrepTool) Execute(ctx context.Context, _ string, args map[string]any) (domain.Observation, error) {
	pattern, ok := stringArg(args, "pattern")
	if !ok || pattern == "" {
